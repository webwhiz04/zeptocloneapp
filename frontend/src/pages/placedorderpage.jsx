...
function PlacedOrderPage() {
    ...
    useEffect(() => {
        ...
        const preferredOrderKey = urlOrderId || getOrderKey(initialOrder);

        if (initialOrder && !urlOrderId) {
            ...
            try {
                ...
                if (normalizedOrders.length > 0) {
                    const matchedOrder = preferredOrderKey
                        ? normalizedOrders.find((item) => 
                            getOrderKey(item) === preferredOrderKey || 
                            item.paymentDetails?.orderId === preferredOrderKey ||
                            item.paymentDetails?.paymentId === preferredOrderKey
                        )
                        : null;
    ...
    }, [email, urlOrderId, location.state?.order]);
                        ...
                        <section className="placedordercard">
                            ...
                            <div className="placedorderitems">
                                {orderItems.map((item) => {
                                    ...
                                    const isHighlighted = urlProductId && String(productId) === String(urlProductId);

                                    return (
                                        <article 
                                            className={`placedorderitem ${isHighlighted ? "highlighted-product" : ""}`} 
